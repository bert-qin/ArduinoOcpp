...
{
    return [fn, connectorId](float power, float current, int nphases)
    { fn(connectorId, power, current, nphases); };
}

std::function<void(float)> adaptFn(unsigned int connectorId, OutputFloat_m fn)
{
    return [fn, connectorId](float value)
    { return fn(connectorId, value); };
}
...
{
    setTxNotificationOutput([notificationOutput](MicroOcpp::ITransaction *tx, MicroOcpp::TxNotification notification)
...
{
    setTxNotificationOutput([notificationOutput, connectorId](MicroOcpp::ITransaction *tx, MicroOcpp::TxNotification notification)
...
{
    setOnResetNotify([onResetNotify](bool isHard)
...
{
    getDiagnosticsService()->setOnUpload([fn](const char *location, MicroOcpp::Timestamp &startTime, MicroOcpp::Timestamp &stopTime)
        {
            return fn(location);
        });
}
...
{
    getDiagnosticsService()->setOnUploadStatusInput([fn]()
        {
        auto v = fn();
        if (v == OptionalTrue)
        {
