...
bool ChargingSchedule::calculateLimit(const Timestamp &t, const Timestamp &startOfCharging, ChargeRate &limit, Timestamp &nextChange)
{
    ...
    if (duration > 0)
    {
        ...
        if (t_toBasis >= duration)
        { //"duration" is given relative to basis
            return false;
        }
        else
        {
...
bool ChargingSchedule::toJson(DynamicJsonDocument &doc, const ProtocolVersion &version)
{
    size_t capacity = 0;
...
void ChargingSchedule::printSchedule()
{
    ...
    MO_CONSOLE_PRINTF("   > CHARGING SCHEDULE:\n"
                      ...
                      duration,
                      tmp,
                      chargingRateUnit == (ChargingRateUnitType::Amp) ? "A" : chargingRateUnit == (ChargingRateUnitType::Watt) ? "W"
                                                                                                                               : "Error",
                      minChargingRate);
...
bool ChargingProfile::toJson(DynamicJsonDocument &doc)
{
    ...
    if (!chargingSchedule.toJson(chargingScheduleDoc,version))
    {
        return false;
    }
...
#if MO_ENABLE_V201
        +MO_TXID_LEN_MAX + 1 + JSON_ARRAY_SIZE(1) + // for str transaction id and
#endif
    ...
    {
        idKey = "id";
        if (strlen(getTransactionIdStr()) > 0)
        {
            doc["transactionId"] = getTransactionIdStr();
        }
    }
    else
#endif
    ...
    if (version.major == 2)
    {
        JsonArray schedule = doc.createNestedArray("chargingSchedule");
        schedule.add(chargingScheduleDoc);
    }
    else
#endif
    {
...
void ChargingProfile::printProfile()
{
    ...
    MO_CONSOLE_PRINTF("   > CHARGING PROFILE:\n"
                      ...
                      transactionId,
                      stackLevel,
                      chargingProfilePurpose == (ChargingProfilePurposeType::ChargePointMaxProfile) ? "ChargePointMaxProfile" : chargingProfilePurpose == (ChargingProfilePurposeType::TxDefaultProfile) ? "TxDefaultProfile"
                                                                                                                            : chargingProfilePurpose == (ChargingProfilePurposeType::TxProfile)          ? "TxProfile"
                                                                                                                                                                                                         : "Error");
    MO_CONSOLE_PRINTF(
        ...
        "   > validFrom: %s\n"
        "   > validTo: %s\n",
        chargingProfileKind == (ChargingProfileKindType::Absolute) ? "Absolute" : chargingProfileKind == (ChargingProfileKindType::Recurring) ? "Recurring"
                                                                              : chargingProfileKind == (ChargingProfileKindType::Relative)    ? "Relative"
                                                                                                                                              : "Error",
        recurrencyKind == (RecurrencyKindType::Daily) ? "Daily" : recurrencyKind == (RecurrencyKindType::Weekly) ? "Weekly"
                                                              : recurrencyKind == (RecurrencyKindType::NOT_SET)  ? "NOT_SET"
                                                                                                                 : "Error",
        tmp,
        tmp2);
...
{
    auto res = std::unique_ptr<ChargingProfile>(new ChargingProfile(version));
    ...
    auto success = loadChargingSchedule(scheduleJson, schedule, version);
    if (!success)
    {
        return nullptr;
    }
...
bool MicroOcpp::loadChargingSchedule(JsonObject &json, ChargingSchedule &out, const ProtocolVersion &version)
{
#if MO_ENABLE_V201
    if (version.major == 2)
    {
