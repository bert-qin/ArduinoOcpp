    ...
    : context(context), model(context.getModel()), filesystem(filesystem), connectorId(connectorId)
{
    ...
    if (model.getVersion().major == 2)
    {
        ...
        transactionMessageAttemptsInt = varService->declareVariable<int>("OCPPCommCtrlr", "MessageAttempts", 3, MO_VARIABLE_FN, Variable::Mutability::ReadWrite, "TransactionEvent");
        transactionMessageRetryIntervalInt = varService->declareVariable<int>("OCPPCommCtrlr", "MessageAttemptInterval", 60, MO_VARIABLE_FN, Variable::Mutability::ReadWrite, "TransactionEvent");
    }
    else
#endif
    {
...
ChargePointStatus Connector::getStatus()
{
    ...
    else
    {
        ...
        if (previous == ChargePointStatus_Finishing ||
            ...
            (transaction && transaction->getStartSync().isRequested()))
        { // transaction process still occupying the connector
...
void Connector::loop()
{
    ...
    if (model.getVersion().major == 1 && transaction)
    { // begin exclusively transaction-related operations
        ...
        if (transaction->isActive() &&
            transaction->isIdTagDeauthorized() && (                                // transaction has been deAuthorized
                                                      ...
                                                      !stopTransactionOnInvalidIdBool || stopTransactionOnInvalidIdBool->getBool()))
        { // if transaction is running, behavior depends on StopTransactionOnInvalidId
        ...
        if (!transaction->isRunning())
        {
            ...
            { // if defined, user Input for allowing StartTx must be true
                // start Transaction
...
std::unique_ptr<Request> Connector::fetchFrontRequest()
{
    ...
    if (transactionFront && !transactionFront->isSilent())
    {
        if (transactionFront->getStartSync().isRequested() && !transactionFront->getStartSync().isConfirmed())
        {
            ...
            transactionFront->commit();
            std::unique_ptr<Request> startTx;
#if MO_ENABLE_V201
            if (model.getVersion().major == 2)
            {
                ...
                {
                    startTx = makeRequest(new Ocpp201::TransactionEvent(model, transaction, true));
                }
            }
            else
#endif
            {
                startTx = makeRequest(new Ocpp16::StartTransaction(model, transactionFront));
            }
            startTx->setOnReceiveConfListener([this](JsonObject response)
                                              {
                                                ...
                                                const char *idTagInfoStatus = nullptr;
#if MO_ENABLE_V201
                                                if (model.getVersion().major == 2){
                                                    idTagInfoStatus = response["idTokenInfo"]["status"] | "_Undefined";
                                                }else
#endif
                                                {
                                                    idTagInfoStatus = response["idTagInfo"]["status"] | "_Undefined";
                                                }
...
#if MO_ENABLE_V201
                                                        if (model.getVersion().major == 2){
                                                            this->getTransaction()->setParentIdTag(response["idTagInfo"]["groupIdToken"]["idToken"] | "");
                                                        }else
#endif
                                                        {
        ...
        if (transactionFront->getStopSync().isRequested() && !transactionFront->getStopSync().isConfirmed())
        {
            ...
            if (model.getVersion().major == 2)
            {
                ...
                {
                    stopTx = makeRequest(new Ocpp201::TransactionEvent(model, transaction, false));
                }
            }
            else
#endif
