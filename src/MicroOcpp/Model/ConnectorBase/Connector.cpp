...
Connector::Connector(Context& context, std::shared_ptr<FilesystemAdapter> filesystem, unsigned int connectorId)
    ...
    if(model.getVersion().major == 2){
        ...
        transactionMessageRetryIntervalInt = transactionMessageAttemptsInt = varService->declareVariable<int>("OCPPCommCtrlr", "MessageAttemptInterval", 60, MO_VARIABLE_FN, Variable::Mutability::ReadWrite,"TransactionEvent");
...
#if MO_ENABLE_V201
            if (model.getVersion().major==2) {
                startTx = makeRequest(new Ocpp201::TransactionEvent(model, std::static_pointer_cast<Ocpp201::Transaction>(transactionFront), true));
            }else
#endif
            {
...
#if MO_ENABLE_V201
            if (model.getVersion().major==2) {
                stopTx = makeRequest(new Ocpp201::TransactionEvent(model, std::static_pointer_cast<Ocpp201::Transaction>(transactionFront), false));
            }else
#endif
            {
